...
let seek = document.getElementsByTagName("seek");
let bar2 = document.getElementById("bar2");
let dot = document.getElementsByClassName("dot")[0];

audioElement.addEventListener("loadedmetadata", () => {
  let audioElement_dur = audioElement.duration;
  let min = Math.floor(audioElement_dur / 60);
  let sec = Math.floor(audioElement_dur % 60);
  if (sec < 10) {
    sec = `0${sec}`;
  }
...
audioElement.addEventListener("timeupdate", () => {
  let audioElement_current = audioElement.currentTime;
  let audioElement_dur = audioElement.duration;

  let min1 = Math.floor(audioElement_current / 60);
  let sec1 = Math.floor(audioElement_current % 60);
  if (sec1 < 10) {
    sec1 = `0${sec1}`;
  }
  ...
  if (audioElement_dur) {
    let progressBar = parseInt((audioElement_current / audioElement_dur) * 100);
